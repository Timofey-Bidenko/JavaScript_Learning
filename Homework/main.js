...
// ✅ >>> Line 97 - 130 <<< function memorize(fn) { ... }
...
// ✅ >>> Line 93 + 122 <<< const cacheLimit = 10 + if (map.size > cacheLimit) { ... }

// Додайте перевірку, щоб прибрати дублікати результатів із кешу.
// ✅ >>> Line 112 <<< if (map.has(key)) { ... } // Не додавав перевірку, бо нема потреби. Кешуватися будуть тільки "нові" значення. "нові" - це ті, які не існують у списку кешу на момент виконання функції.
